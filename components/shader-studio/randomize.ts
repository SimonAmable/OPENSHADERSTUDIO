...
function randomSurfaceAndMotion() {
  return {
...
export function remixRecipe(_recipe: Recipe): Partial<Recipe> {
  return randomSurfaceAndMotion();
}
...
export const VARIATION_MODE_META: Record<VariationMode, { label: string; hint: string }> = {
  ...
  remix: { label: "Remix", hint: "Keep style and colours; new settings" },
