...
Feed the shader its u_resolution, u_time, u_pointer, u_velocity, u_colors, style, palette, surface, movement, frame, and cursor uniforms from the selected recipe.`;
  ...
  return <main className="studio-shell" onDragOver={(event) => event.preventDefault()} onDrop={handleDrop}>
    ...
    <section className="workspace">
      <nav className="icon-rail" aria-label="Shader controls">{tabs.map(({ id, label, icon: Icon }) => <button key={id} className={`rail-tab ${tab === id ? "active" : ""}`} onClick={() => { if (id === "export") { setExportTab("image"); setExportOpen(true); return; } setTab(id); }} aria-label={label}><Icon size={19} strokeWidth={1.8} /><span>{label}</span></button>)}</nav>
