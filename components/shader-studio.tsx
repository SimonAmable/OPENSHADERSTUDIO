...
type AnimationClip = { id: string; label: string; presetId: string; start: number; duration: number };
...
export function ShaderStudio() {
  ...
  useEffect(() => {
    ...
    setMotionPreview(animationClips.some((clip) => playhead >= clip.start && playhead <= clip.start + clip.duration) ? "focus" : "base");
  }, [animationClips, editorMode, playhead]);
...
Feed the shader its u_resolution, u_time, u_pointer, u_velocity, u_colors, style, palette, surface, movement, frame, and cursor uniforms from the selected recipe.`;
  ...
  const focusPreviewState = { ...mockup, ...focusPreset.settings, scale: focusPreset.settings.scale * focusZoom, tiltX: focusPreset.settings.tiltX + focusTilt };
  ...
  const openAnimation = () => {
    ...
    setAnimationClips((clips) => {
      ...
      const first = { id: crypto.randomUUID(), label: "Zoom", presetId: focusPresetId, start: 0, duration: animationDuration };
      setActiveClipId(first.id); return [first];
    });
    setEditorMode("animation"); setMotionPreview("base");
  };
  const addAnimationClip = () => {
    const last = animationClips.at(-1);
    const duration = Math.min(animationDuration, baseDuration);
    const clip = { id: crypto.randomUUID(), label: cameraMode === "tilt" ? "Tilt" : "Zoom", presetId: focusPresetId, start: last ? Math.min(baseDuration - duration, last.start + last.duration + .2) : 0, duration };
  ...
  const selectAnimationClip = (clip: AnimationClip) => { setActiveClipId(clip.id); setFocusPresetId(clip.presetId); setEditorMode("animation"); setMotionPreview("focus"); };
  ...
  const beginClipGesture = (event: PointerEvent<HTMLElement>, clip: AnimationClip, kind: "move" | "resize") => {
    ...
    const move = (next: globalThis.PointerEvent) => {
      ...
      setAnimationClips((clips) => clips.map((item) => {
        ...
        if (gesture.kind === "resize") return { ...item, duration: Math.round(Math.max(.6, Math.min(baseDuration - item.start, gesture.duration + delta)) * 10) / 10 };
        return { ...item, start: Math.round(Math.max(0, Math.min(baseDuration - item.duration, gesture.start + delta)) * 10) / 10 };
      }));
    };
  ...
  const previewPreset = mockupPresets.find((preset) => preset.id === (previewClip?.presetId ?? focusPresetId)) ?? focusPreset;
  const stageMockup = editorMode === "animation" && motionPreview === "focus"
    ? { ...mockup, ...previewPreset.settings, scale: previewPreset.settings.scale * focusZoom, tiltX: previewPreset.settings.tiltX + focusTilt }
    : mockup;
  ...
  return <main className={`studio-shell ${editorMode === "animation" ? "animation-mode" : ""}`} onDragOver={(event) => event.preventDefault()} onDrop={handleDrop}>
    ...
    <section className="workspace">
      ...
      {tab === "mockup" && <><div className="mockup-viewport"><div className={`mockup-stage frame-${mockup.frame}`} style={{ transform: `translate(calc(-50% + ${stageMockup.x}%), calc(-50% + ${stageMockup.y}%)) scale(${stageMockup.scale}) perspective(1200px) rotateX(${stageMockup.tiltX}deg) rotateY(${stageMockup.tiltY}deg) rotateZ(${stageMockup.rotate}deg)`, borderRadius: mockup.radius, boxShadow: `0 ${18 + mockup.shadow / 3}px ${35 + mockup.shadow}px rgba(0,0,0,${.2 + mockup.shadow / 160})`, visibility: mockup.visible ? "visible" : "hidden" }}><div className="browser-bar"><i /><i /><i /><span>your-product.com</span></div>{mockup.media ? <img src={mockup.media} alt="Mockup preview" /> : <div className="mockup-demo"><span>THE NEXT RELEASE</span><h1>Make the work<br />feel inevitable.</h1><p>Your product, framed by a live visual system.</p><b>Explore release notes <span>↗</span></b></div>}</div></div><aside className="camera-inspector"><div className="camera-tabs"><button className={cameraMode === "zoom" ? "active" : ""} onClick={() => setCameraMode("zoom")}>Zoom</button><button className={cameraMode === "tilt" ? "active" : ""} onClick={() => setCameraMode("tilt")}>Tilt</button></div><div className="camera-pad" onPointerDown={moveCamera} onPointerMove={(event) => event.buttons === 1 && moveCamera(event)} role="application" aria-label="Camera positioning pad"><div className="camera-pad-card" style={{ transform: `translate(-50%, -50%) rotate(${mockup.rotate}deg) scale(${.65 + mockup.scale * .18})` }} /><span className="camera-cross horizontal" /><span className="camera-cross vertical" /><i className="camera-handle" style={{ left: `${cameraMode === "zoom" ? 50 + mockup.x * 1.78 : 50 + mockup.tiltY * 4.15}%`, top: `${cameraMode === "zoom" ? 50 + mockup.y * 2.38 : 50 - mockup.tiltX * 5.55}%` }} /></div><Slider label={cameraMode === "zoom" ? "Zoom" : "Tilt"} value={cameraMode === "zoom" ? mockup.scale : mockup.tiltY} min={cameraMode === "zoom" ? .45 : -12} max={cameraMode === "zoom" ? 1.2 : 12} step={.01} unit={cameraMode === "zoom" ? "×" : "°"} onChange={(value) => updateMockup(cameraMode === "zoom" ? { scale: value } : { tiltY: value })} /><div className="section-label camera-label">Layout presets</div><div className="layout-presets">{mockupPresets.map((preset) => <button key={preset.id} onClick={() => useMockupPreset(preset)} className={`layout-preset ${preset.id}`}><span className="layout-backdrop" /><i style={{ transform: `translate(${preset.settings.x}%, ${preset.settings.y}%) rotate(${preset.settings.rotate}deg) scale(${preset.settings.scale})` }} />{preset.id === "hero" && <b>Custom layout</b>}<em>{preset.label}</em></button>)}</div></aside></>}
      {tab === "mockup" && <>
        {editorMode === "animation" && <aside className="motion-inspector">
          <div className="motion-header"><div><span className="eyebrow">SELECTED ANIMATION</span><h2>{activeClip?.label ?? "Animation"}</h2><p>Choose its destination, then fine-tune it.</p></div><button onClick={selectBaseMedia}>Edit base</button></div>
          <Slider label="Clip length" value={activeClip?.duration ?? animationDuration} min={.6} max={baseDuration} step={.1} unit="s" onChange={(duration) => { setAnimationDuration(duration); setAnimationClips((clips) => clips.map((clip) => clip.id === activeClipId ? { ...clip, duration: Math.min(duration, baseDuration - clip.start) } : clip)); }} />
          ...
          <button className="motion-focus-preview" onClick={() => setMotionPreview((value) => value === "focus" ? "base" : "focus")}><span>Destination preview</span><i style={{ transform: `perspective(260px) rotateX(${focusPreviewState.tiltX}deg) rotateY(${focusPreviewState.tiltY}deg) rotate(${focusPreviewState.rotate}deg) scale(${focusPreviewState.scale})` }} /></button>
          <Slider label={cameraMode === "zoom" ? "Zoom" : "Tilt"} value={cameraMode === "zoom" ? focusZoom : focusTilt} min={cameraMode === "zoom" ? .75 : -16} max={cameraMode === "zoom" ? 1.35 : 16} step={.01} unit={cameraMode === "zoom" ? "×" : "°"} onChange={cameraMode === "zoom" ? setFocusZoom : setFocusTilt} />
          <AnimatePresence>{precisionOpen && <motion.div className="precision-controls" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}><Slider label="Hold" value={animationHold} min={0} max={3} step={.1} unit="s" onChange={setAnimationHold} /></motion.div>}</AnimatePresence>
          <div className="section-label camera-label">Destination preset</div><div className="layout-presets">{mockupPresets.map((preset) => <button key={preset.id} onClick={() => { setFocusPresetId(preset.id); setAnimationClips((clips) => clips.map((clip) => clip.id === activeClipId ? { ...clip, presetId: preset.id, label: cameraMode === "tilt" ? "Tilt" : "Zoom", duration: animationDuration } : clip)); setMotionPreview("focus"); }} className={`layout-preset ${preset.id} ${focusPresetId === preset.id ? "selected" : ""}`}><span className="layout-backdrop" /><i style={{ transform: `translate(${preset.settings.x}%, ${preset.settings.y}%) rotate(${preset.settings.rotate}deg) scale(${preset.settings.scale})` }} /><em>{preset.id === basePresetId ? "Same as base" : preset.label}</em></button>)}</div>
        </aside>}
        <AnimatePresence>
