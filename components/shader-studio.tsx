...
type AnimationClip = { id: string; label: string; presetId: string; start: number; duration: number; transition: number; easing: "ease" | "spring"; zoom: number; tilt: number; hold: number; springSpeed: number; targetX: number; targetY: number; targetTiltX: number; targetTiltY: number; targetRotate: number; exit: "base" | "next" };
...
Feed the shader its u_resolution, u_time, u_pointer, u_velocity, u_colors, style, palette, surface, movement, frame, and cursor uniforms from the selected recipe.`;
  ...
  const createAnimationClip = (start: number, duration = Math.min(animationDuration, baseDuration)): AnimationClip => ({
    ...
    targetX: focusPreset.settings.x, targetY: focusPreset.settings.y, targetTiltX: focusPreset.settings.tiltX, targetTiltY: focusPreset.settings.tiltY, targetRotate: focusPreset.settings.rotate, exit: "base",
  });
  const openAnimation = () => {
  ...
  const targetState = (clip: AnimationClip): MockupSettings => {
    ...
    return { ...mockup, scale: preset.settings.scale * clip.zoom, x: clip.targetX, y: clip.targetY, tiltX: clip.targetTiltX + clip.tilt, tiltY: clip.targetTiltY, rotate: clip.targetRotate };
  };
  ...
  const moveAnimationCamera = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const updated = cameraMode === "zoom"
      ? { ...activeClip, targetX: Math.round((px - .5) * 56), targetY: Math.round((py - .5) * 42) }
      : { ...activeClip, targetTiltY: Math.round((px - .5) * 24), targetTiltX: Math.round((.5 - py) * 24) };
    ...
    <section className="workspace">
      ...
      {tab === "mockup" && <>
        {editorMode === "animation" && <aside className="motion-inspector">
          ...
          <div className="motion-easing-control"><span>Motion curve</span><div><button className={activeClip?.easing === "ease" ? "active" : ""} onClick={() => { if (!activeClip) return; const updated = { ...activeClip, easing: "ease" as const }; setAnimationEasing("ease"); setAnimationClips((clips) => clips.map((clip) => clip.id === activeClipId ? updated : clip)); seekToClipTarget(updated); setToast("Ease motion applied"); }}>Ease</button><button className={activeClip?.easing === "spring" ? "active" : ""} onClick={() => { if (!activeClip) return; const updated = { ...activeClip, easing: "spring" as const }; setAnimationEasing("spring"); setAnimationClips((clips) => clips.map((clip) => clip.id === activeClipId ? updated : clip)); seekToClipTarget(updated); setToast("Spring motion applied"); }}>Spring</button></div></div>
          ...
          <div className="camera-pad animation-camera-pad" onPointerDown={moveAnimationCamera} onPointerMove={(event) => event.buttons === 1 && moveAnimationCamera(event)} role="application" aria-label="Animation destination camera pad"><div className="camera-pad-card" style={{ transform: `translate(-50%, -50%) rotate(${activeClip?.targetRotate ?? 0}deg) scale(${.65 + activeTargetScale * .18})` }} /><span className="camera-cross horizontal" /><span className="camera-cross vertical" /><i className="camera-handle" style={{ left: `${cameraMode === "zoom" ? 50 + (activeClip?.targetX ?? 0) * 1.78 : 50 + (activeClip?.targetTiltY ?? 0) * 2.08}%`, top: `${cameraMode === "zoom" ? 50 + (activeClip?.targetY ?? 0) * 2.38 : 50 - (activeClip?.targetTiltX ?? 0) * 2.08}%` }} /></div>
          <Slider label={cameraMode === "zoom" ? "Zoom" : "Tilt"} value={cameraMode === "zoom" ? focusZoom : focusTilt} min={cameraMode === "zoom" ? .75 : -16} max={cameraMode === "zoom" ? 1.35 : 16} step={.01} unit={cameraMode === "zoom" ? "×" : "°"} onChange={(value) => { if (!activeClip) return; if (cameraMode === "zoom") { const updated = { ...activeClip, zoom: value }; setFocusZoom(value); setAnimationClips((clips) => clips.map((clip) => clip.id === activeClipId ? updated : clip)); seekToClipTarget(updated); } else { const updated = { ...activeClip, tilt: value }; setFocusTilt(value); setAnimationClips((clips) => clips.map((clip) => clip.id === activeClipId ? updated : clip)); seekToClipTarget(updated); } }} />
