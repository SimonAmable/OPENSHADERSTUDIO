...
function CameraPadScene({ recipe, mockup, scale, x, y, rotate, cameraX = mockup.cameraX, cameraY = mockup.cameraY }: { recipe: Recipe; mockup: MockupSettings; scale: number; x: number; y: number; rotate: number; cameraX?: number; cameraY?: number }) {
  const zoomDistance = Math.max(0, scale - 1);
  const focusX = -cameraX * zoomDistance * .5;
  const focusY = -cameraY * zoomDistance * .5;
  const cardScale = Math.max(.42, scale * .66);
  return <>
    <ShaderCanvas recipe={recipe} paused={false} onError={() => undefined} />
    <div className="camera-preview-media" style={{ transform: `translate(calc(-50% + ${(x + focusX) * .36}%), calc(-50% + ${(y + focusY) * .36}%)) rotate(${rotate}deg) scale(${cardScale})` }}>
